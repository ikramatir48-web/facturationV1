...
const S = {
  page: { fontFamily:'Arial,sans-serif', fontSize:12, color:'#000', background:'#fff', padding:'20px 28px', minHeight:'297mm', display:'flex', flexDirection:'column' },
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      ...
      <div className="modal" style={{ maxWidth:820, background:'white', color:'#000', maxHeight:'90vh', overflowY:'auto' }} onClick={e => e.stopPropagation()}>
        ...
        <div id="pz" style={S.page}>{children}</div>
      </div>
    </div>
  )
}
...
function SocieteBloc() {
  return (
    <div style={{ border:'1.5px solid #000', padding:'10px 14px' }}>
      <div style={{ fontWeight:900, fontSize:16, textDecoration:'underline', color:'#000', marginBottom:6 }}>{SOCIETE.nom}</div>
      <div style={{ fontSize:11, color:'#000', lineHeight:1.8 }}>
        <div>{SOCIETE.adresse}</div>
        <div>Tél : {SOCIETE.tel}</div>
        <div>IF : {SOCIETE.if_fiscal} &nbsp;·&nbsp; ICE : {SOCIETE.ice}</div>
        <div>Patente : {SOCIETE.patente} &nbsp;·&nbsp; RC : {SOCIETE.rc}</div>
      </div>
    </div>
  )
}
...
function ClientBloc({ client, label = 'LIVRÉ À :' }) {
  return (
    <div style={{ border:'1.5px solid #000', padding:'10px 14px' }}>
      <div style={{ fontWeight:700, fontSize:11, textTransform:'uppercase', marginBottom:6, color:'#000' }}>{label}</div>
      <div style={{ fontWeight:700, color:'#000', fontSize:13 }}>{client?.nom_societe || client?.nom}</div>
      {client?.nom_societe && <div style={{ fontSize:11, color:'#000' }}>{client.nom}</div>}
      ...
      <div style={{ fontSize:11, color:'#000' }}>N° Client : {client?.numero_client}</div>
    </div>
  )
}
...
function DocFooter() {
  return (
    <div style={{ borderTop:'1px solid #aaa', marginTop:'auto', paddingTop:10, textAlign:'center', fontSize:10, color:'#555', lineHeight:1.7 }}>
      {SOCIETE.nom} &nbsp;·&nbsp; {SOCIETE.adresse}<br/>
      TEL : {SOCIETE.tel} &nbsp;·&nbsp; PATENTE N° {SOCIETE.patente} &nbsp;·&nbsp; IF {SOCIETE.if_fiscal} &nbsp;·&nbsp; I.C.E : {SOCIETE.ice} &nbsp;·&nbsp; R.C N° {SOCIETE.rc}
    </div>
  )
}
  ...
  return (
    <DocModal titre={`Bon de Livraison — ${bl?.numero_bl}`} onClose={onClose}>
      ...
      <div style={{ textAlign:'center', marginBottom:24 }}>
        <div style={{ display:'inline-block', background:'#1a5c8a', color:'white', padding:'10px 40px', fontWeight:700, fontSize:16, letterSpacing:2 }}>
          BON DE LIVRAISON
        </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:16 }}>
        <div>
          <div style={{ fontWeight:700, fontSize:11, textTransform:'uppercase', marginBottom:6, color:'#000' }}>FACTURÉ À :</div>
          <SocieteBloc />
        </div>
        <div>
          <div style={{ fontWeight:700, fontSize:11, textTransform:'uppercase', marginBottom:6, color:'#000' }}>LIVRÉ À :</div>
          <ClientBloc client={client} label="LIVRÉ À :" />
        </div>
      </div>

      {/* Ref commande + condition */}
      <div style={{ display:'flex', justifyContent:'space-between', fontSize:11, color:'#000', marginBottom:14, padding:'6px 0', borderTop:'1px solid #ddd', borderBottom:'1px solid #ddd' }}>
      ...
      {/* Livreur */}
      {livreur ? (
        <div style={{ fontSize:12, color:'#000', marginBottom:20, padding:'8px 12px', border:'1px solid #ddd', borderRadius:4 }}>
        ...
        <div style={{ fontSize:11, color:'#aaa', marginBottom:20 }}>Aucun livreur assigné</div>
      )}

      {/* Signatures + QR — répartis sur toute la largeur */}
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:20, marginTop:16, alignItems:'end' }}>
        <div>
          <div style={{ fontSize:11, color:'#555', marginBottom:6 }}>Signature Réceptionnaire :</div>
          <div style={{ border:'1px solid #999', height:80 }}></div>
        </div>
        <div>
          <div style={{ fontSize:11, color:'#555', marginBottom:6 }}>Signature Chauffeur :</div>
          <div style={{ border:'1px solid #999', height:80 }}></div>
        </div>
        <div style={{ textAlign:'center' }}>
          <QRCode value={qrData} size={90} />
          <div style={{ fontSize:10, color:'#444', marginTop:3 }}>GED: {bl?.numero_bl}</div>
...
export function PrintBC({ commande, lignes, client, tva = 10, onClose }) {
  ...
  return (
    <DocModal titre={`Bon de Commande — ${commande?.numero_commande}`} onClose={onClose}>
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', marginBottom:20 }}>
        <table style={{ borderCollapse:'collapse' }}>
          <tbody>
            <tr><td style={{ border:'1px solid #000', padding:'7px 14px', textAlign:'right', fontWeight:600, minWidth:140, background:'#f5f5f5', fontSize:12, color:'#000' }}>Montant HT</td><td style={{ border:'1px solid #000', padding:'7px 14px', textAlign:'right', minWidth:130, fontSize:12, color:'#000' }}>{totalHT.toFixed(2)} DH</td></tr>
            <tr><td style={{ border:'1px solid #000', padding:'7px 14px', textAlign:'right', fontWeight:600, background:'#f5f5f5', fontSize:12, color:'#000' }}>TVA {tva}%</td><td style={{ border:'1px solid #000', padding:'7px 14px', textAlign:'right', fontSize:12, color:'#000' }}>{montantTVA.toFixed(2)} DH</td></tr>
            <tr><td style={{ border:'1px solid #000', padding:'7px 14px', textAlign:'right', fontWeight:700, background:'#e8e8e8', fontSize:12, color:'#000' }}>Total TTC</td><td style={{ border:'1px solid #000', padding:'7px 14px', textAlign:'right', fontWeight:700, background:'#e8e8e8', fontSize:12, color:'#000' }}>{totalTTC.toFixed(2)} DH</td></tr>
          </tbody>
        </table>
      </div>

      <div style={{ fontSize:12, color:'#000', marginBottom:24, padding:'10px 0', borderTop:'1px solid #eee' }}>
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', marginTop:'auto' }}>
        <div style={{ textAlign:'center' }}>
          ...
          <div style={{ fontSize:10, color:'#444', marginTop:3 }}>GED: {commande?.numero_commande}</div>
        </div>
      </div>
...
// FACTURE
// ═══════════════════════════════════════════
export function PrintFacture({ facture, bl, commande, lignes, client, tva = 10, onClose }) {
  const lignesTri  = trierLignes(lignes)
  const totalTTC   = lignesTri.reduce((s,l) => s + l.quantite * l.prix_unitaire, 0)
  const totalHT    = totalTTC / (1 + tva/100)
  const montantTVA = totalTTC - totalHT
  ...
  return (
    <DocModal titre={`Facture — ${facture?.numero_facture}`} onClose={onClose}>
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', marginTop:'auto', paddingTop:16 }}>
        <div style={{ textAlign:'center' }}>
          <QRCode value={qrData} size={80} />
          <div style={{ fontSize:10, color:'#444', marginTop:3 }}>GED: {facture?.numero_facture}</div>
          <div style={{ fontSize:10, color:'#444' }}>Exemplaire Client</div>
        </div>
      </div>
