...
export async function emailConfirmationCommande(to, nom, numeroCommande, produits, total, dateLivraison) {
  const produitsHtml = produits.map(p =>
    `<tr>
      <td style="padding:8px 12px;border-bottom:1px solid #f0f0f0;">${p.nom}</td>
  ...
  await sendEmail(
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#f9f9f9;border-radius:8px;">
      ...
      ${dateLivraison ? `<div style="background:#e3f2fd;border-left:4px solid #1976d2;padding:12px 16px;border-radius:4px;margin-bottom:20px;"><strong>Livraison souhaitée :</strong> ${dateLivraison}</div>` : ''}
...
export async function emailFactureDisponible(to, nom, numeroCommande, numeroFacture, montant) {
  await sendEmail(
    to,
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:32px;background:#f9f9f9;border-radius:8px;">
      ...
      <div style="background:#fff;border:1px solid #e0e0e0;border-radius:8px;padding:20px;margin:24px 0;text-align:center;">
        ...
        <div style="font-size:32px;font-weight:700;color:#e85d04;">${Number(montant).toFixed(2)} DH</div>
      </div>
